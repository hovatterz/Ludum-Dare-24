module Roguevolution
  class FOV
    ...
    def calculate(position, radius)
      @start_x, @start_y = position.x, position.y
      @radius_sq = radius * radius

      # We always see the center
      @dungeon.tile_at(@start_x, @start_y).light
    ...
    def visit_coord(x, y, dx, dy, view_index, active_views)
      ...
      @dungeon.tile_at(coord[0], coord[1]).light
      ...
      return unless @dungeon.tile_at(coord[0], coord[1]).transparent == false
